...
void __stdcall ImageCallBackEx(unsigned char * pData, MV_FRAME_OUT_INFO_EX* pFrameInfo, void* pUser)
{
    ...
    // 将图像数据转换为 OpenCV 格式（Bayer RG8）
    cv::Mat bayerImage(pFrameInfo->nExtendHeight, pFrameInfo->nExtendWidth, CV_8UC1, pData);

    // 将 Bayer RG8 转换为 RGB8
    cv::Mat rgbImage;
...
{
    ros::init(argc, argv, "camera_publisher0");
    ros::NodeHandle nh;

    // 创建 ROS 图像发布器，发布到 "/camera/image" 话题
    image_pub = nh.advertise<sensor_msgs::Image>("camera/image0", 1);
    ...
    do 
    {
        ...
        {
            //MV_CC_DEVICE_INFO* p_cam_Info = stDeviceList.pDeviceInfo[0];

            for (int i = 0; i < stDeviceList.nDeviceNum; i++)
            {
                ROS_INFO("[device %d]:", i);
                //std::cout<<"SN:  "<<p_cam_Info->SpecialInfo.stGigEInfo.chSerialNumber<<std::endl;
